...
@section('content')
    @php
        ...
        $linkedDiscountAmount = $payment->ajustesProveedor->whereNull('anulado_at')->sum('monto');
    ...
    <section class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Resumen del pago">
        ...
        <div class="border-l-4 border-hazard bg-paper p-5 shadow-sm"><p class="font-mono text-[8px] tracking-wider text-steel-500 uppercase">Total pagado vigente</p><p class="mt-2 font-display text-3xl font-extrabold text-ink-950">{{ $money($balance->total_pagado) }}</p>@if ($linkedDiscountAmount > 0)<p class="mt-1 text-xs text-steel-500">Más {{ $money($linkedDiscountAmount) }} de descuento reconocido</p>@endif</div>
        <div class="border-l-4 border-danger bg-paper p-5 shadow-sm"><p class="font-mono text-[8px] tracking-wider text-danger uppercase">Deuda total del proveedor</p><p class="mt-2 font-display text-3xl font-extrabold text-ink-950">{{ $money($providerDebt) }}</p><p class="mt-1 text-xs text-steel-500">Suma de todas sus cargas pendientes</p></div>
    ...
    <div class="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
        ...
        <aside class="grid content-start gap-6">
            <section class="reveal-up reveal-up-delay-2 border border-line bg-paper shadow-panel" aria-labelledby="payment-load-title">
                ...
                <div class="p-5"><p class="font-mono text-xs font-semibold text-signal">{{ $payment->carga->numero_carga }}</p><p class="mt-3 font-display text-2xl font-bold text-ink-950 uppercase">{{ $payment->carga->proveedor->nombre_razon_social }}</p><p class="mt-2 text-sm text-steel-500">{{ $payment->carga->producto->nombre }} · recibida el {{ $payment->carga->fecha_carga->format('d/m/Y') }}</p><p class="mt-2 break-all font-mono text-xs text-steel-500">Cuenta / CCI: {{ $payment->carga->proveedor->numero_cuenta ?: 'No registrada' }}</p><div class="mt-5 grid grid-cols-2 gap-3"><div class="border border-line bg-canvas p-4"><p class="font-mono text-[8px] text-steel-500 uppercase">Costo</p><p class="mt-2 font-display text-lg font-bold text-ink-950">{{ $money($payment->carga->costo_total) }}</p></div><div class="border border-line bg-canvas p-4"><p class="font-mono text-[8px] text-steel-500 uppercase">Pendiente de esta carga</p><p class="mt-2 font-display text-lg font-bold text-danger">{{ $money($balance->saldo_pendiente) }}</p></div></div><div class="mt-5 grid gap-2"><a href="{{ route('cargas-proveedor.show', $payment->carga) }}" class="inline-flex min-h-11 w-full items-center justify-center border border-ink-950 font-display text-xs font-bold tracking-wider text-ink-950 uppercase transition hover:bg-ink-950 hover:text-white">Ver carga completa</a>@if ($authenticatedUser?->tienePermiso('REPORTES_VER'))<a href="{{ route('reportes.supplier-account', $payment->carga->proveedor) }}" class="inline-flex min-h-11 w-full items-center justify-center bg-ink-950 font-display text-xs font-bold tracking-wider text-white uppercase transition hover:bg-signal">Ver cuenta del proveedor</a>@endif</div></div>
            </section>
