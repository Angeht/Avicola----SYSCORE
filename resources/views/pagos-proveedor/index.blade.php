...
@section('content')
    ...
    <header class="reveal-up flex flex-col gap-6 border-b border-line pb-7 xl:flex-row xl:items-end xl:justify-between">
        <div class="max-w-3xl">
            ...
            <p class="mt-3 max-w-2xl text-sm leading-6 text-steel-500">Consulta la deuda acumulada de cada proveedor y los abonos distribuidos entre sus cargas.</p>
        </div>
    ...
    <section class="reveal-up reveal-up-delay-1 mt-6 overflow-hidden border border-line bg-paper shadow-panel" aria-labelledby="supplier-debts-title">
        <div class="flex flex-col gap-3 border-b border-line px-5 py-5 sm:flex-row sm:items-end sm:justify-between sm:px-6"><div><p class="font-mono text-[9px] font-semibold tracking-[0.18em] text-signal uppercase">Cuenta consolidada / Proveedores</p><h2 id="supplier-debts-title" class="mt-1 font-display text-2xl font-bold text-ink-950 uppercase">Deuda acumulada</h2><p class="mt-2 text-sm text-steel-500">Cada fila reúne todas las cargas que todavía tienen saldo.</p></div>@if ($authenticatedUser?->tienePermiso('REPORTES_VER'))<a href="{{ route('reportes.show', 'deudas-proveedores') }}" class="inline-flex min-h-10 items-center justify-center border border-ink-950 px-4 font-display text-xs font-bold tracking-wider text-ink-950 uppercase transition hover:bg-ink-950 hover:text-white">Reporte general</a>@endif</div>
        @if ($supplierDebts->isEmpty())
            <x-empty-state title="Sin deudas pendientes" description="Todos los proveedores se encuentran pagados." />
        @else
            <div class="hidden overflow-x-auto md:block"><table class="w-full min-w-[920px] border-collapse text-left" aria-label="Deuda acumulada por proveedor"><thead class="bg-canvas font-mono text-[8px] tracking-wider text-steel-500 uppercase"><tr><th class="px-5 py-3 font-semibold">Proveedor</th><th class="px-5 py-3 font-semibold">Cuenta / CCI</th><th class="px-5 py-3 text-right font-semibold">Cargas</th><th class="px-5 py-3 text-right font-semibold">Costo</th><th class="px-5 py-3 text-right font-semibold">Abonado / ajustado</th><th class="px-5 py-3 text-right font-semibold">Deuda total</th><th class="px-5 py-3 text-right font-semibold">Acción</th></tr></thead><tbody class="divide-y divide-line">@foreach ($supplierDebts as $supplierDebt)<tr><td class="px-5 py-4 font-semibold text-ink-950">{{ $supplierDebt->proveedor }}</td><td class="px-5 py-4 font-mono text-xs text-ink-700">{{ $supplierDebt->numero_cuenta ?: 'No registrada' }}</td><td class="px-5 py-4 text-right font-mono text-xs text-ink-700">{{ $quantity($supplierDebt->cargas_pendientes) }}</td><td class="px-5 py-4 text-right font-mono text-xs text-ink-700">{{ $money($supplierDebt->costo_total) }}</td><td class="px-5 py-4 text-right font-mono text-xs text-signal">{{ $money(((float) $supplierDebt->total_pagado) + ((float) $supplierDebt->total_ajustado)) }}</td><td class="px-5 py-4 text-right font-display text-xl font-bold text-danger">{{ $money($supplierDebt->deuda_total) }}</td><td class="px-5 py-4 text-right"><div class="flex justify-end gap-2">@if ($authenticatedUser?->tienePermiso('REPORTES_VER'))<a href="{{ route('reportes.supplier-account', $supplierDebt->proveedor_id) }}" class="inline-flex min-h-9 items-center border border-ink-950 px-3 font-mono text-[9px] font-semibold tracking-wider text-ink-950 uppercase transition hover:bg-ink-950 hover:text-white">Detalle</a>@endif<a href="{{ route('pagos-proveedor.create', ['proveedor' => $supplierDebt->proveedor_id]) }}" class="inline-flex min-h-9 items-center bg-ink-950 px-3 font-mono text-[9px] font-semibold tracking-wider text-white uppercase transition hover:bg-signal">Pagar</a></div></td></tr>@endforeach</tbody></table></div>
            <div class="divide-y divide-line md:hidden">@foreach ($supplierDebts as $supplierDebt)<article class="p-5"><div class="flex items-start justify-between gap-4"><div><h3 class="font-semibold text-ink-950">{{ $supplierDebt->proveedor }}</h3><p class="mt-1 break-all font-mono text-xs text-steel-500">{{ $supplierDebt->numero_cuenta ?: 'Cuenta no registrada' }}</p></div><span class="border border-hazard/40 bg-hazard-soft px-2 py-1 font-mono text-[8px] font-semibold uppercase">{{ $quantity($supplierDebt->cargas_pendientes) }} cargas</span></div><div class="mt-4 grid grid-cols-2 gap-3 border-y border-line py-4"><div><p class="font-mono text-[8px] text-steel-500 uppercase">Abonado / ajustado</p><p class="mt-1 font-semibold text-signal">{{ $money(((float) $supplierDebt->total_pagado) + ((float) $supplierDebt->total_ajustado)) }}</p></div><div class="text-right"><p class="font-mono text-[8px] text-danger uppercase">Deuda total</p><p class="mt-1 font-display text-xl font-bold text-danger">{{ $money($supplierDebt->deuda_total) }}</p></div></div><div class="mt-4 grid gap-2">@if ($authenticatedUser?->tienePermiso('REPORTES_VER'))<a href="{{ route('reportes.supplier-account', $supplierDebt->proveedor_id) }}" class="inline-flex min-h-10 w-full items-center justify-center border border-ink-950 font-display text-xs font-bold tracking-wider text-ink-950 uppercase">Ver detalle</a>@endif<a href="{{ route('pagos-proveedor.create', ['proveedor' => $supplierDebt->proveedor_id]) }}" class="inline-flex min-h-10 w-full items-center justify-center bg-ink-950 font-display text-xs font-bold tracking-wider text-white uppercase">Registrar pago</a></div></article>@endforeach</div>
        @endif
    </section>

    <form method="GET" action="{{ route('pagos-proveedor.index') }}" class="reveal-up reveal-up-delay-1 mt-6 grid gap-3 border border-line bg-paper p-4 shadow-sm xl:grid-cols-[minmax(0,1fr)_220px_170px_190px_auto]">
        ...
        <div><label for="payment-provider" class="sr-only">Proveedor</label><select id="payment-provider" name="proveedor" class="min-h-12 w-full border border-line bg-white px-4 text-sm font-semibold text-ink-700 outline-none transition focus:border-ink-950 focus:ring-2 focus:ring-hazard/40"><option value="">Todos los proveedores</option>@foreach ($providers as $provider)<option value="{{ $provider->id }}" @selected((string) $selectedProviderId === (string) $provider->id)>{{ $provider->nombre_razon_social }}</option>@endforeach</select></div>
        ...
        <div class="flex gap-2"><button type="submit" class="inline-flex min-h-12 flex-1 items-center justify-center bg-ink-950 px-5 font-display text-sm font-bold tracking-wider text-white uppercase transition hover:bg-ink-800 xl:flex-none">Filtrar</button>@if ($search !== '' || $selectedProviderId || $status !== 'vigentes' || $date !== today()->toDateString())<a href="{{ route('pagos-proveedor.index') }}" class="inline-flex min-h-12 items-center justify-center border border-line px-4 text-xs font-semibold text-steel-500 uppercase transition hover:border-ink-950 hover:text-ink-950" aria-label="Limpiar filtros">×</a>@endif</div>
    </form>
