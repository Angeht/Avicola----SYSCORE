...
@php
    ...
    $compactPrice = static function (float|int|string|null $value, string $decimalSeparator = ',', string $thousandsSeparator = '.'): string {
        $formatted = number_format((float) ($value ?? 0), 4, $decimalSeparator, $thousandsSeparator);

        return preg_replace('/0{1,2}$/', '', $formatted) ?? $formatted;
    };
    $appliedPrice = $detail['precio_aplicado_kg'] ?? '';
@endphp
