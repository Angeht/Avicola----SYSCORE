...
<html lang="es">
    <head>
        ...
        <title>Estado de cuenta · {{ $cliente->nombres_razon_social }} · {{ config('app.name') }}</title>
        <style>
            ...
            .client { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-top: 12px; }
            .client div, .summary article { border: 1px solid #d9d8ce; padding: 10px; }
            .client strong, .summary strong { display: block; margin-top: 5px; font-size: 14px; }
    ...
    <body>
        ...
        <div class="actions">
            ...
            <a href="{{ route('reportes.customer-account', ['cliente' => $cliente, 'hasta' => $cutoff]) }}">Volver al estado de cuenta</a>
        </div>

        <header>
            <div>
                <p class="label muted">Finanzas / Estado de cuenta</p>
                <h1>{{ $cliente->nombres_razon_social }}</h1>
        ...
        <section class="client" aria-label="Datos del cliente">
            <div><p class="label muted">Documento</p><strong>{{ $cliente->nro_documento ?: 'No registrado' }}</strong></div>
            <div><p class="label muted">Teléfono</p><strong>{{ $cliente->telefono ?: 'No registrado' }}</strong></div>
            <div><p class="label muted">Dirección</p><strong>{{ $cliente->direccion ?: 'No registrada' }}</strong></div>
        </section>
        ...
        <table aria-label="Movimientos del estado de cuenta">
            <thead>
                <tr>
                    <th>Fecha</th>
                    <th>Movimiento</th>
            ...
            <tbody>
                ...
                @empty
                    <tr><td colspan="7">No hay movimientos hasta la fecha de corte seleccionada.</td></tr>
                @endforelse
            </tbody>
        </table>
    </body>
