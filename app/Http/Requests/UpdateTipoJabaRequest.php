...
class UpdateTipoJabaRequest extends FormRequest
{
    ...
    {
        $crateType = $this->route('tipoJaba');
        $uniqueName = Rule::unique('tipos_jaba', 'nombre');

        if ($crateType instanceof TipoJaba) {
            $uniqueName->ignore($crateType);
        }

        return [
            'nombre' => ['required', 'string', 'max:80', $uniqueName],
